# 1.2. Datastations als hoeksteen voor een decentraal, hybride BVO

## 1.2.1. Datastations en het zandloper model
Het concept van **datastations** is een van de twee pijlers van dit document dat een architectuur beschrijft van een landelijk dekkend hybride BVO ten behoeve van secundair gebruik. Het concept van datastations kent vandaag de dag veel verschillende verschijningsvormen:
...
- De FAIR principes zijn uitgewerkt in het concept van een [FAIR data point](https://specs.fairdatapoint.org/fdp-specs-v1.2.html), zijnde een datastation gevuld met FAIR metadata van de data dat is bedoeld als een gefedereerde oplossing voor een data catalogus.
...
Het zandloper model gaat uit van vijf lagen die de data moeten laten stromen vanaf het eerste moment dat ze worden vastgelegd door de data houder (laag 1) tot en met het uiteindelijke secundair gebruik door de datagebruiker (laag 5).
...
??? abstract "Data product"
    ...
    Elk dataproduct bevat, naast de data zelf, ook metadata. Deze metadata beschrijven onder andere de structuur van de data, de inhoudelijke eisen waaraan de data voldoet, en verwijzingen naar de betekenis van de data (vastgelegd in een ontologie of domeinmodel). Daarnaast legt een dataproduct vast aan welke regels de datagebruiker moet voldoen voor toegang tot de data: hoe het mag worden gebruikt en welke beleidsregels van toepassing zijn voor toegang.
